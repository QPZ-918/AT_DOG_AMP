...
class MotionDataTerm(ManagerTermBase):
    ...
    def _get_motion_target_articulation(self):
        for asset_name in ("robot", "robot_anim"):
            try:
                return self._env.scene[asset_name]
            except Exception:
                continue
        return None

    def _reorder_motion_dofs_to_articulation_order(self, dof_pos: torch.Tensor) -> torch.Tensor:
        articulation = self._get_motion_target_articulation()
        if articulation is None:
            return dof_pos

        motion_joint_names = getattr(articulation.cfg, "joint_sdk_names", None)
        if not motion_joint_names:
            return dof_pos

        articulation_joint_names = list(getattr(articulation, "joint_names", []))
        if not articulation_joint_names:
            return dof_pos

        if motion_joint_names == articulation_joint_names:
            return dof_pos

        if set(motion_joint_names) != set(articulation_joint_names):
            raise ValueError(
                "Motion joint names do not match articulation joint names. "
                f"motion={motion_joint_names}, articulation={articulation_joint_names}"
            )

        reorder_indices = [motion_joint_names.index(joint_name) for joint_name in articulation_joint_names]
    ...
    def _load_motion_data(self):
        ...
        for motion_name, motion_weight in self.motion_weights_dict.items():
            ...
            dof_pos = self._reorder_motion_dofs_to_articulation_order(dof_pos)
