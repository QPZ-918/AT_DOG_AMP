...
def reorder_dof_pos_to_articulation_order(dof_pos: torch.Tensor, articulation: Articulation) -> torch.Tensor:
...
def run_simulator(
    ...
    for motion_data in motion_data_dicts:
        ...
        dof_pos_tensor = torch.from_numpy(motion_data["dof_pos"]).to(scene.device).float()
        dof_pos_tensor = reorder_dof_pos_to_articulation_order(dof_pos_tensor, robot)
        dof_pos_list.append(dof_pos_tensor)
