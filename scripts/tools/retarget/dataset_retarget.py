...
Behavior:
 ...
 - Saves each converted motion dict to the output directory with the same filename, or with an optional suffix.
...
parser.add_argument(
    ...
    type=str,
    default="",
    help="Optional suffix appended to each output filename before .pkl (example: _walk)",
)
parser.add_argument(
    "--loop",
    type=str,
...
elif args_cli.robot == "atdog2":
    from legged_lab.assets.atdog2 import ATDOG2_CFG as ROBOT_CFG
else:
...
def make_output_path(output_dir: str, input_path: Path, output_suffix: str) -> Path:
    return Path(output_dir) / f"{input_path.stem}{output_suffix}.pkl"
...
def main():
    ...
    for name, motion in zip(input_names, motion_data_dicts):
        ...
        out_path = make_output_path(args_cli.output_dir, input_path, args_cli.output_suffix)
